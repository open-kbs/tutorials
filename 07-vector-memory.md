...
**Archive when context shifts:**
- Project completes → archive project memory
- Campaign ends → archive campaign data
- Season changes → archive seasonal strategies
- Strategy pivots → archive old approach

**Archive proactively:**
- Before memory fills up (~80 items)
- When information is no longer actively used
- To preserve snapshots at milestones

**Search archive before:**
- Starting a similar project
- Planning seasonal campaigns
- Making decisions that past experience could inform

## 7.9 Example Workflow

```
User: "Start planning our summer sale campaign"

Agent thinking:
1. Search archive for past summer campaigns
2. Find relevant historical data
3. Use insights to inform new planning

Agent:
<searchArchive>
{"query": "summer sale campaign promotions discounts", "topK": 5}
</searchArchive>

[Results show last year's summer campaign data]

Agent: "Based on last summer's campaign, I found that..."
```
...
Agent:
<archiveItems>
["memory_summer_2024_campaign", "memory_summer_2024_results", "memory_summer_2024_learnings"]
</archiveItems>

"Archived summer 2024 campaign data. Ready to start fresh for fall planning."
```
...
Test:
1. Create some memory items
2. Ask agent to archive them
3. Search for archived content using natural language
4. Verify semantic search finds relevant items even with different wording

## Summary

- Two-tier memory: Active (limited, always loaded) + Archive (unlimited, searchable)
- `embeddingModel` and `embeddingDimension` in settings enable VectorDB
- `archiveItems` moves items to VectorDB with embeddings
- `searchArchive` finds items by meaning, not exact match
- Archive when context shifts, search before similar work
- Timestamp in archive ID allows multiple snapshots of same item

## Next

The complete agent code is in [agents/telegram-agent/](./agents/telegram-agent/).

For more OpenKBS features, see the [OpenKBS Documentation](https://github.com/open-kbs/openkbs).
